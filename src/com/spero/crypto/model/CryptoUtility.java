...
public class CryptoUtility {
	...
	public byte[] encryptWithAESAndIV(String keyString, String ivString, byte[] rawData) {
		...
		try {
			IvParameterSpec iv = new IvParameterSpec(ivString.getBytes("UTF-8"));
			/*
			 * Random rand = new SecureRandom(); byte[] bytes = new byte[16];
			 * rand.nextBytes(bytes); IvParameterSpec iv = new IvParameterSpec(bytes);
			 */
			SecretKeySpec skeySpec = new SecretKeySpec(keyString.getBytes("UTF-8"), "AES");
			 ...
			 * catch (InvalidAlgorithmParameterException var11) { throw new
			 * AsymmetricEncryptionFailure(var11); }
			 */ catch (UnsupportedEncodingException var12) {
			throw new AsymmetricEncryptionFailure(var12);
		}
	...
	public PublicKey getPublic(String filePath) throws Exception {
		...
		String content = "";
		/*
		 * //Test Start String privateKeyBeginning = "-----BEGIN PRIVATE KEY-----\n";
		 * String privateKeyEnding = "-----END PRIVATE KEY-----"; String
		 * publicKeyBeginning = "-----BEGIN PUBLIC KEY-----\n"; String publicKeyEnding =
		 * "-----END PUBLIC KEY-----"; if (content.contains(privateKeyBeginning)) {
		 * content = content.replace(privateKeyBeginning, ""); }
		 * 
		 * if (content.contains(publicKeyBeginning)) { content =
		 * content.replace(publicKeyBeginning, ""); }
		 * 
		 * if (content.contains(privateKeyEnding)) { content =
		 * content.replace(privateKeyEnding, ""); }
		 * 
		 * if (content.contains(publicKeyEnding)) { content =
		 * content.replace(publicKeyEnding, ""); }
		 * 
		 * content = content.replaceAll("\\s", ""); // Test End
		 */
