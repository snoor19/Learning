...
public class Payment {
	...
	public void pay() {
		...
		try {
			...
			byte[] rawEncryptedBytes = crypto.encrypt(crypto.getPublic(this.gwPublicKeyPath),
					rawDataToBeEncrypted.getBytes("UTF-8"));
			byte[] rawSignatureBytes = crypto.sign(crypto.getPrivate(this.privateKeyPath),
					rawDataToBeEncrypted.getBytes("UTF-8"));
			...
			byte[] rawDecryptedBytes = crypto.decrypt(crypto.getPrivate(privateKeyPath),
					decoder.decode(response.getSensitiveData()));
			String responseSensitiveData = new String(rawDecryptedBytes, Charset.forName("UTF-8"));
			ObjectMapper objectMapper = new ObjectMapper();
			SensitiveDataInitializeResponse sensitiveDataInitializeResponse = (SensitiveDataInitializeResponse) objectMapper
					.readValue(responseSensitiveData, SensitiveDataInitializeResponse.class);
			System.out.println("Initialization Response Sensitive Data: " + responseSensitiveData.toString());
			Boolean verify = crypto.verifySign(crypto.getPublic(this.gwPublicKeyPath), rawDecryptedBytes,
					decoder.decode(response.getSignature()));
			...
			rawEncryptedBytes = crypto.encrypt(crypto.getPublic(this.gwPublicKeyPath),
					rawDataToBeEncrypted.getBytes("UTF-8"));
			rawSignatureBytes = crypto.sign(crypto.getPrivate(this.privateKeyPath),
					rawDataToBeEncrypted.getBytes("UTF-8"));
			...
			String body1 = "{     \"sensitiveData\": \"" + sensitiveData + "\",     \"signature\": \"" + signature
					+ "\",     \"merchantCallbackURL\": \"http://sandbox.mynagad.com:10707/merchant-server/web/confirm\",     \"additionalMerchantInfo\": {      \"productName\":\"shirt\",      \"productCount\":1     } } ";
		} catch (Exception e) {
	...
	public void cancel() {
		...
		try {
			...
			String body = "{     \"sensitiveDataCancelRequest\": \"" + sensitiveData + "\",     \"signature\": \""
					+ signature + "\"} ";
